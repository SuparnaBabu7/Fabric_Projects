...
get_ipython().system('pip install azure-eventhub #install azure eventhub')


# In[3]:


import json          # Import JSON library to serialize traffic event data into JSON format
import random        # Import random library to generate synthetic traffic values (vehicles, congestion, etc.)
import time          # Import time library to control event generation frequency with sleep intervals
import uuid          # Import UUID library to create unique identifiers for each traffic event
from datetime import datetime   # Import datetime to timestamp each traffic event with current date and time
from azure.eventhub import EventHubProducerClient, EventData  
# Import Azure Event Hub client classes to connect to Event Hub and send event data

# CONFIGURATION: BENGALURU TRAFFIC STREAM

EVENTSTREAM_CONNECTION_STRING = "Endpoint=sb://esehpn4xl8xtndkqserp85.servicebus.windows.net/;SharedAccessKeyName=key_5e605dca-c877-4490-a661-edc29d13f3c6;SharedAccessKey=7B0Xfw2dXPWnodF4t+G2V3wFHqeZcpnDG+AEhGeFweY=;EntityPath=esehpn4xl8xtndkqserp85_eh"
# Connection string for Azure Event Hub namespace (contains endpoint and shared access key for authentication)

EVENTSTREAM_HUB_NAME = "esehpn4xl8xtndkqserp85_eh"
# Name of the specific Event Hub within the namespace where traffic events will be published
...
# ============================================================
# Bengaluru Traffic Telemetry Simulator
# ------------------------------------------------------------
# This block generates synthetic real-time traffic data for 
# multiple junctions in Bengaluru. It models rush-hour severity, 
# average speeds, signal wait times, congestion index, and vehicle 
# mix distribution. The telemetry events are streamed to Azure 
# Event Hub every 10 seconds, enabling live dashboards in Fabric 
